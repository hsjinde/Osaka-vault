...
## 2026-07-16（新增景點：勝尾寺）
- 使用者要求「達摩公園」旅遊攻略；vault 內查無此名稱，經確認為大阪箕面市「勝尾寺」（俗稱達摩寺）
- 新建 `wiki/entities/景點/勝尾寺.md`：因無對應本地剪藏，內容為網路查證彙整（WebSearch，多篇部落格/官網/巴士時刻表交叉比對），涵蓋地址／門票／開放時間、兩條巴士轉乘路線（箕面萱野／千里中央）、達摩祈願玩法、賞楓（11月中–12月上）與繡球花（6月上–7月中）時期、與箕面大滝的健行連通（非同站，不建議當順遊）
- source_type 未標注（非 entity 標準流程，來源非本地剪藏），source_count: 0，`## 來源` 改列外部參考連結
- ⚠️ 圖片：此 session 缺 `CLOUDFLARE_API_TOKEN`，無法依 core_rules 上傳 R2，本頁暫無圖片，已於頁首標注待補
- 更新 `wiki/index.md` 景點區塊新增索引列，`updated` → 2026-07-16
- 觸發關鍵字：使用者於對話要求「幫我產生一個達摩公園的旅遊攻略，盡可能附上圖片」
