...
## 2026-09-07 - 每日行程合理性檢視與時段重排

- **觸發來源**：使用者要求「看每日行程安排的合理性並規劃路線」，並指定**不刪除任何既有項目**
- **作法**：所有判斷為衝突／撲空風險的項目一律改列 `備選` 時段，不從檔案移除；新增 4 格補上原本缺的交通與正餐
- **操作內容**：
  - 更新 `wiki/dashboard/每日行程.md`（49 → 53 格，0 刪除）
    - Day 0：Shake Shack 與寶可夢 DX 同排 16:30，Shake Shack 改列備選；補計程車 4 人以上需 2 台；標注燒肉訂 2 位、Day 2 鰻魚訂 6 位的人數矛盾待確認
    - Day 1：新增「心齋橋 → USJ」交通格與「園內或 City Walk 晚餐」；Hommachi-seimenjo（無營業時間記錄、21:10 才到）改列備選
    - Day 2：下午改為大丸 → 露天神社 → HEP FIVE → 扭蛋之森 → LUCUA 的單向順序，去掉原本往南又折返的一段；補注江坂以北屬北大阪急行、Metro 一日券不涵蓋
    - Day 3：新增「新世界午餐」與「晚餐（邊逛邊吃）」兩格（原本中午沒獨立時段、晚上只有蛋糕）；黑門提前到 15:50 結束，才趕得上難波八阪 17:00 截止
    - Day 4：宮田麵兒（11:30 才開、與出發衝突）改列備選；OBRA 與最後補貨補上 11:00-11:20 並標二選一
  - 更新 `wiki/concepts/交通票券攻略.md`：Day 2 逐日拆解改為實際行程；Amazing Pass 結案為「不買」（行程已無海遊館與大阪城）；回程時間與每日行程 Day 4 對齊；待確認清單前兩項結案
- ⚠️ 未查證項目（標 📌 留在頁面上，未自行填補）：Os Drug 心齋橋店、扭蛋之森梅田茶屋町店、露天神社的營業時間，實體頁皆無記錄；USJ 10/01 開園時刻依季節浮動
- 查證來源：[北大阪急行 FAQ](https://www.kita-kyu.co.jp/faq/)、[エンジョイエコカード利用範圍](https://osaka-subway.com/enjoyecocard/)（確認一日券不含江坂以北）

## 2026-09-07 - 訂位人數結案：兩筆都是 2 位

- **觸發來源**：使用者回覆「都是訂 2 位」，解掉前一筆記錄中標為待確認的人數矛盾
- **操作內容**：
  - `wiki/dashboard/每日行程.md`：Day 2 Unagi Kushiyaki Izumo 訂位人數 6 位 → **2 位**（原為筆誤）
  - 移除 Day 0 燒肉力丸那格的人數矛盾 ⚠ 註記
  - Day 0 Check-in 的計程車註記改為「2 人一台就夠」（原本按人數不明寫的 4 人以上叫 2 台）
  - Day 3 備選新世界串炸的排隊策略改為 2 人版本（好插吧台空位，週六抓 30 分）
